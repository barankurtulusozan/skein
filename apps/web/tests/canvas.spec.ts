...
test.describe("Skein Canvas E2E Tests", () => {
  ...
  test("should allow dragging and dropping nodes onto the canvas", async ({
    page,
  }) => {
    ...
    await expect(page.locator(".react-flow__node-manual-trigger")).toHaveCount(
      1,
    );
    await expect(page.locator(".react-flow__node-http-request")).toHaveCount(1);
  });

  test("should validate port connection types and show toast errors", async ({
    page,
  }) => {
    const canvas = page.locator(".react-flow__pane");
  ...
  test("should connect compatible ports and persist on page reload", async ({
    page,
  }) => {
    const canvas = page.locator(".react-flow__pane");
