...
export const NODE_DEFINITIONS: Record<string, UINodeDefinition> = {
  ...
  "webhook-trigger": {
    ...
    configFields: [],
    description:
      "Listen on an HTTP endpoint and start a run when a POST is received.",
  },
  "schedule-trigger": {
    ...
    category: "trigger",
    inputs: [],
    outputs: [{ id: "timestamp", label: "Timestamp", dataType: "number" }],
    configFields: [
      {
        name: "cron",
  ...
  delay: {
    ...
    label: "Delay",
    category: "action",
    inputs: [{ id: "input", label: "Trigger", dataType: "any" }],
    outputs: [{ id: "output", label: "Triggered", dataType: "any" }],
    configFields: [
      {
        name: "seconds",
  ...
  condition: {
    ...
    configFields: [
      {
        name: "expression",
        label: "Expression (e.g. input === true)",
        type: "text",
        defaultValue: "input === true",
      },
    ],
    description:
      "Branch execution path based on boolean Javascript expression.",
  },
  loop: {
    type: "loop",
    label: "Loop (For Each)",
    category: "logic",
    inputs: [{ id: "array", label: "Array", dataType: "array" }],
    outputs: [
    ...
    configFields: [],
    description:
      "Iterate over an array input, running downstream actions per item.",
  },
  transform: {
    type: "transform",
    ...
    configFields: [
      {
        ...
        type: "textarea",
        defaultValue: "// Javascript: (input) => output\nreturn input;",
      },
    ],
  ...
  "llm-prompt": {
    ...
    configFields: [
      ...
      { name: "model", label: "Model", type: "text", defaultValue: "gpt-4o" },
      {
        ...
        type: "textarea",
        defaultValue: "Synthesize the following input: {{input}}",
      },
    ],
    description:
      "Call a vendor-agnostic large language model completions endpoint.",
  },
  "tool-call": {
    type: "tool-call",
    label: "Tool Call (Function)",
    category: "ai",
    inputs: [{ id: "args", label: "Arguments", dataType: "object" }],
    ...
    description:
      "Register a downstream node as a callable function tool for an LLM node.",
  },
  "log-debug": {
    type: "log-debug",
    label: "Log / Debug",
    category: "output",
    inputs: [{ id: "data", label: "Log Data", dataType: "any" }],
    outputs: [],
    configFields: [],
    description:
      "Output inputs to logs and visual interface execution console.",
  },
};
