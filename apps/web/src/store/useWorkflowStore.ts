...
export const useWorkflowStore = create<WorkflowState>((set, get) => {
  ...
  return {
    ...
    onEdgesChange: (changes) => {
      ...
      if (serializeState(get().nodes, oldEdges) !== serializeState(get().nodes, newEdges)) {
      ...
      set((state) => {
        ...
        if (lastHistory) {
          const positionsChanged = state.nodes.some((node) => {
            ...
            return !histNode || histNode.position.x !== node.position.x || histNode.position.y !== node.position.y;
          });
          if (!positionsChanged) return {};
        }
        return {
          past: [...state.past, { nodes: JSON.parse(JSON.stringify(state.nodes)), edges: JSON.parse(JSON.stringify(state.edges)) }],
          future: [],
        };
      });
    },
