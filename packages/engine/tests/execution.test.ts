...
describe("WorkflowExecutor & Topological Sort", () => {
  describe("topoSort", () => {
    ...
    it("should throw an error on cycles", () => {
      ...
      expect(() => topoSort(workflow)).toThrow(/Cycle detected/);
    });
  });
  ...
  describe("execute", () => {
    it("should execute a simple linear workflow successfully", async () => {
      const workflow = {
        ...
        nodes: [
          ...
          },
          {
            ...
            type: "transform",
            position: { x: 0, y: 0 },
            config: { code: "return input.message.toUpperCase();" },
          },
          {
            id: "logger",
            type: "log-debug",
      ...
      expect(results.trigger.output).toEqual({
